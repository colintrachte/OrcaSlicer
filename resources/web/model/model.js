...
function AddScrollEvent() {
  ...
  $("#Info_Inside_Board").scroll(function () {
    ...
    let BBottom = Math.abs(
      BItem.offset().top + BItem.innerHeight() - ParentItem.offset().top,
    );
    let ATop = Math.abs(AItem.offset().top - ParentItem.offset().top);
    let ABottom = Math.abs(
      AItem.offset().top + AItem.innerHeight() - ParentItem.offset().top,
    );
    let PTop = Math.abs(PItem.offset().top - ParentItem.offset().top);
    let PBottom = Math.abs(
      PItem.offset().top + PItem.innerHeight() - ParentItem.offset().top,
    );
...
function ShowModelInfo(pModel) {
  ...
  if (TotalPreview > 0) {
    ...
    for (let pn = 0; pn < TotalPreview; pn++) {
      ...
      htmlPreview +=
        '<div class="swiper-slide"><img class="Model_PrevImg" src="' +
        FTmpPath +
        '" /></div>';
    }
  ...
  for (let f = 0; f < fTotal; f++) {
    ...
    if (strClass != "ImageIcon") {
      strHtml +=
        ...
        "</div>" +
        "	</div>" +
        '	<div class="FileMenu" onClick="OnClickOpenFile(\'' +
        tPath +
        '\')"><img src="img/s.svg" /></div>' +
        "</div>";
    } else {
      ImgID++;
      ...
      strHtml +=
        ...
        "</div>" +
        "	</div>" +
        '	<div class="FileMenu" onClick="OnClickOpenImage(\'' +
        TmpImgID +
        '\')"><img src="img/s.svg" /></div>' +
        "</div>";
    }
