...
function GetFullToday() {
  ...
  var strM = nmonth + "";
  if (nmonth < 10) strM = "0" + nmonth;

  var strD = nday + "";
  if (nday < 10) strD = "0" + nday;
...
function GetFullDate() {
  ...
  if (tDate.nmonth < 10) strM = "0" + tDate.nmonth;

  var strD = tDate.nday + "";
  if (tDate.nday < 10) strD = "0" + tDate.nday;

  var strH = tDate.nhour + "";
  if (tDate.nhour < 10) strH = "0" + tDate.nhour;

  var strMin = tDate.nminute + "";
  if (tDate.nminute < 10) strMin = "0" + tDate.nminute;

  var strS = tDate.nsecond + "";
  if (tDate.nsecond < 10) strS = "0" + tDate.nsecond;
...
Array.prototype.in_array = function (e) {
  ...
  if (sArray.indexOf(skey) >= 0) return true;
  else return false;
};
...
function ExecuteDarkMode(DarkCssPath) {
  ...
  if (bDarkMode != null) nMode = 1;

  let nNow = CheckCssLinkExist(DarkCssPath);
  if (nMode == 0) {
    if (nNow > 0) RemoveCssLink(DarkCssPath);
  } else {
    if (nNow == 0) AddCssLink(DarkCssPath);
  }
}
