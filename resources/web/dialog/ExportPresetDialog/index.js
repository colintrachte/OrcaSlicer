...
function InstallInputSafeKeydown() {
  ...
  document.onkeydown = function (event) {
    ...
    if (e && e.keyCode === 27 && typeof ClosePage === "function") ClosePage();
...
function SyncListFromDom(listId, store) {
  var map = {};
  for (var n = 0; n < store.length; n++) map[store[n].id] = store[n];

  $("#" + listId + " .CValues input:gt(0)").each(function () {
    var id = String($(this).attr("data-key") || "");
    if (map[id]) map[id].checked = !!$(this).prop("checked");
  });
}
...
function ToggleNoItems(listId, show) {
  ...
  if (show) node.addClass("show");
  else node.removeClass("show");
}
...
function SendMessage(command, data) {
  ...
  if (typeof SendWXMessage === "function") SendWXMessage(JSON.stringify(msg));
}
