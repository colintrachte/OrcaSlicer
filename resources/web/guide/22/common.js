...
function SortUI() {
  ...
  for (let n = 0; n < nMode; n++) {
    ...
    if (OneMode["nozzle_selected"] != "") ModelList.push(OneMode);
  }
  ...
  for (let n = 0; n < nMode; n++) {
    ...
    HtmlMode +=
      '<label><input type="checkbox" mode="' +
      sModel["model"] +
      '"  nozzle="' +
      sModel["nozzle_selected"] +
      '"   onChange="MachineClick()" /><span>' +
      sModel["model"] +
      "</span></label>";
  }
  ...
  for (let n = 0; n < SortedFilamentArray.length; n++) {
    ...
    if (bFind) {
      ...
      if (!TypeHtmlArray.hasOwnProperty(LowType)) {
        /* ORCA use label tag to allow checkbox to toggle when user ckicked to text */
        let HtmlType =
          '<label><input type="checkbox" filatype="' +
          fType +
          '" onChange="FilaClick()"   /><span>' +
          fType +
          "</span></label>";
      ...
      if (!VendorHtmlArray.hasOwnProperty(lowVendor)) {
        /* ORCA use label tag to allow checkbox to toggle when user ckicked to text */
        let HtmlVendor =
          '<label><input type="checkbox" vendor="' +
          fVendor +
          '"  onChange="VendorClick()" /><span>' +
          fVendor +
          "</span></label>";
      ...
      if (pFila.length == 0) {
        /* ORCA use label tag to allow checkbox to toggle when user ckicked to text */
        let HtmlFila =
          '<label class="MItem"><input type="checkbox" onChange="UpdateStats()" vendor="' +
          fVendor +
          '"  filatype="' +
          fType +
          '" filalist="' +
        ...
        if (strModel == "" || fModel == "") pFila.attr("model", "");
        else pFila.attr("model", strModel + fModel);
  ...
  if (SelectNumber == 0) ChooseDefaultFilament();
...
function SortFilament() {
  ...
  for (let m = 0; m < nFilament; m++) {
    ...
    if (TypeList.in_array(fType) && VendorList.in_array(fVendor)) {
      ...
      if (HasModel || fModel == "") $(OneNode).show();
      else $(OneNode).hide();
    } else {
      $(OneNode).hide();
...
function UpdateStats() {
  ...
  let $filteredItems = $visibleItems.filter(function () {
...
function SelectAllFilament(nShow) {
  ...
  if (document.querySelector(".cbr-filter-bar").value) {
    $("#ItemBlockArea .MItem:visible input")
      .filter(function () {
        return $(this).closest(".MItem").css("position") !== "absolute";
      })
...
function ChooseDefaultFilament() {
  ...
  for (let m = 0; m < nFilament; m++) {
    ...
    if (HasModel) $(OneFF).prop("checked", true);
  }
...
function initInputEvents() {
  document.querySelectorAll(".cbr-search-bar").forEach((searchBar) => {
    searchBar.addEventListener("input", function () {
      ...
      list.querySelector(".cbr-no-items").style.display =
        hidden === items.length ? "block" : "none";
    });
