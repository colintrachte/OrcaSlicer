...
function InstallNetworkPlugin() {
  if (
    m_ProfileItem["network_plugin_install"] != "1" ||
    (m_ProfileItem["network_plugin_install"] == "1" &&
      m_ProfileItem["network_plugin_compability"] == "0")
  ) {
    $("#AcceptBtn").hide();
...
function ReturnPreviewPage() {
  ...
  if (nMode == 1) document.location.href = "../1/index.html";
  else document.location.href = "../21/index.html";
}

function GotoNetPluginPage() {
  let bRet = ResponseFilamentResult();

  if (bRet) window.location.href = "../4orca/index.html";
}
