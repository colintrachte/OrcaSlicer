...
function HandleModelList(pVal) {
  if (!pVal.hasOwnProperty("model")) return;
  ...
  for (let n = 0; n < nTotal; n++) {
    ...
    if (!ModelHtml.hasOwnProperty(strVendor)) ModelHtml[strVendor] = "";
  ...
  for (let key in ModelHtml) {
    $(".OneVendorBlock[vendor='" + key + "'] .PrinterArea").append(
      ModelHtml[key],
    );
  }
...
function ChooseModel(vendor, ModelName) {
  let ChooseItem = $(
    ".ModelCheckBox[vendor='" + vendor + "'][model='" + ModelName + "']",
  );

  if (ChooseItem.length > 0) {
    if ($(ChooseItem).hasClass("ModelCheckBoxSelected"))
      $(ChooseItem).removeClass("ModelCheckBoxSelected");
    else $(ChooseItem).addClass("ModelCheckBoxSelected");

    SetModelSelect(
      vendor,
      ModelName,
      $(ChooseItem).hasClass("ModelCheckBoxSelected"),
    );
  }
}
...
function FilterModelList(keyword) {
  ...
  for (let n = 0; n < ModelSelect.length; n++) {
    ...
    SetModelSelect(
      strVendor,
      strModel,
      $(OneItem).hasClass("ModelCheckBoxSelected"),
    );
  }
  ...
  for (let n = 0; n < nTotal; n++) {
    ...
    if (!kwSplit.every((s) => search.includes(s))) continue;
  ...
  for (let n = 0; n < ModelSelect.length; n++) {
    ...
    if (checked) $(OneItem).addClass("ModelCheckBoxSelected");
    else $(OneItem).removeClass("ModelCheckBoxSelected");
  }
  ...
  $content.css(
    "padding-right",
    $content[0].scrollHeight > $content[0].clientHeight ? "10px" : "20px",
  );
...
function CreateVendorBlock(vendorName) {
  let alt = vendorName;
  if (alt == "BBL") alt = "Bambu Lab";
  if (alt == "Custom") alt = "Custom Printer";
  if (alt == "Other") alt = "Orca colosseum";

  return (
    '<div class="OneVendorBlock" Vendor="' +
    vendorName +
    '">' +
    ...
    alt +
    "</a>" +
    '		<div class="BannerBtns" onClick="ChooseVendor(' +
    "\'" +
    vendorName +
    "\'" +
    ')">' +
...
function CreatePrinterBlock(OneModel) {
  ...
  if (vendor == "Custom") modelName = modelName.split(" ")[1];
  ...
  return (
    '<div class="PrinterBlock" onClick="ChooseModel(\'' +
    vendor +
    "','" +
    OneModel["model"] +
    "')\">" +
    '	<div class="PImg">' +
    '		<img class="ModelThumbnail" src="' +
    OneModel["cover"] +
    '" />' +
    "	</div>" +
    ...
    '		<div class="ModelCheckBox" vendor="' +
    vendor +
    '" model="' +
    OneModel["model"] +
    '"></div>' +
...
function OnExitFilter() {
  ...
  for (let vendor in ModelNozzleSelected) {
    for (let model in ModelNozzleSelected[vendor]) {
      if (!ModelNozzleSelected[vendor][model]) continue;
...
function findSnap(cur, dir) {
  if (lastSnapTarget !== null && Math.abs(cur - lastSnapTarget) < SNAP_CORR)
    return null;
  ...
  const cardGap =
    firstCard && firstArea ? bcTop(firstCard) - bcTop(firstArea) : 0;
  ...
  for (const el of candidates) {
    ...
    if (snapTo != lastSeen) {
      lastSeen = snapTo;
      if (dir === "down" && snapTo > cur + SNAP_CORR) {
        result = snapTo;
        break;
      }
      if (dir === "up" && snapTo < cur - SNAP_CORR) {
        result = snapTo;
        break;
      }
    }
...
function initScrollEvents() {
  ...
  $content[0].addEventListener(
    ...
    (e) => {
      ...
      if (Math.abs(dy) > 3) scrollDir = dy > 0 ? "down" : "up";
    },
    { passive: true },
  );
  ...
  document.addEventListener("keydown", (e) => {
    if (document.activeElement != SearchBox) {
      ...
      if (scrollKeys.includes(e.key) || (hasFocus && e.which == 9)) armSnap();
    }
  });
...
function LayoutMode(value) {
  ...
  if ($content[0].getAttribute("layout") === value) return;
...
function initKeyEvents(closeOnESC) {
  document.onkeydown = function (event) {
    ...
    if (e.keyCode == 27) {
      if (sidebar.getAttribute("open") == "1") {
        // prefer to close sidebar first if its open
